...
L1TrendFiltering::L1TrendFiltering(int n, int i_k) {
  if (i_k >= 2) {
    MoMALogger::message(
        "TF with higher-than-second difference matrix is not well tested yet.");
  }
  if (n == -1) {
    MoMALogger::error(
        "Class L1TrendFiltering needs to know dimension of the problem.");
  }
...
L1TrendFiltering::~L1TrendFiltering() {
  MoMALogger::debug(
      "Releasing a L1 linear trend filtering proximal operator object");
}

// Find the sum of dual residual and centering residual
double res(const arma::mat &DDt, const arma::vec &Dy, const arma::vec &mu1,
           const arma::vec &mu2, const arma::vec &f1, const arma::vec &f2,
           double t, const arma::vec &nu) {
...
arma::vec L1TrendFiltering::operator()(const arma::vec &x, double l) {
  ...
  arma::vec nu = arma::zeros<arma::vec>(
      m);  // The dual variable, y - D^t*nu is what we need
  arma::vec mu1 =
      arma::ones<arma::vec>(m);  // Multipliers to solve the dual problem
  ...
  for (; iter < MAX_ITER; iter++) {
    ...
    arma::mat part1 =
        DDt - arma::diagmat(mu1 / f1 + mu2 / f2);  // A band matrix
    arma::vec part2 = -DDt * nu + Dy + (1 / t) / f1 - (1 / t) / f2;

    // Update directions
    arma::vec dnu = arma::solve(
        part1,
  ...
  double dres = 0.5 * arma::as_scalar(Dymm.t() * arma::solve(DDt, Dymm)) +
                l * arma::sum(mu1 + mu2);
  arma::vec Dtnu = D.t() * nu;
  double pres =
      -0.5 * arma::as_scalar(Dtnu.t() * Dtnu) + arma::as_scalar(Dy.t() * nu);
