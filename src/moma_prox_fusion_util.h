...
class Group {
 public:
  ...
  Group(int h = -1, int t = -1, int p = -1, double lambda = -1,
        double beta = -1, double slope = 0)
      : head(h), tail(t), parent(p), lambda(lambda), beta(beta), slope(slope){};
  void print() {
    MoMALogger::debug("") << "[" << head << "," << tail
...
class FusedGroups {
 public:
  ...
  double lines_meet_at(double x1, double x2, double k1, double k2, double y1,
                       double y2);
