...
class _PR_solver {
 protected:
  ...
  ProxOp p;
  // A gradient operator
  arma::vec g(const arma::vec &v, const arma::vec &y, double step_size,
              const arma::mat &S, bool is_S_idmat);
...
class ISTA : public _PR_solver {
 public:
  ISTA(double i_alpha, const arma::mat &i_Omega, double i_lambda,
       Rcpp::List prox_arg_list, double i_EPS, int i_MAX_ITER, int dim)
      : _PR_solver(i_alpha, i_Omega, i_lambda, prox_arg_list, i_EPS, i_MAX_ITER,
                   dim) {
...
class FISTA : public _PR_solver {
 public:
  FISTA(double i_alpha, const arma::mat &i_Omega, double i_lambda,
        Rcpp::List prox_arg_list, double i_EPS, int i_MAX_ITER, int dim)
      : _PR_solver(i_alpha, i_Omega, i_lambda, prox_arg_list, i_EPS, i_MAX_ITER,
                   dim) {
...
class OneStepISTA : public _PR_solver {
 public:
  OneStepISTA(double i_alpha, const arma::mat &i_Omega, double i_lambda,
              Rcpp::List prox_arg_list, double i_EPS, int i_MAX_ITER, int dim)
      : _PR_solver(i_alpha, i_Omega, i_lambda, prox_arg_list, i_EPS, i_MAX_ITER,
                   dim) {
