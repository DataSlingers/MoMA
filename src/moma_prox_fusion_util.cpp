...
FusedGroups::FusedGroups(const arma::vec &x) : heap(x.n_elem - 1) {
  ...
  for (int i = 0; i < heap.heap_storage.size(); i++) {
    ...
    double h = 0;
    h = lines_meet_at(0, 0, g[i + 1].slope, g[i].slope, g[i + 1].beta,
                      g[i].beta);
...
void FusedGroups::print() {
  ...
  for (int i = 0; i < g.size(); i++) {
    if (is_valid(i)) {
      g[i].print();
      if (g[i].map_to_heap != NOT_IN_HEAP &&
          g[i].map_to_heap >= heap.heap_storage.size()) {
        MoMALogger::error("Exceeds heap limit")
            << g[i].map_to_heap << "while heap size is "
            << heap.heap_storage.size();
      }
    } else {
      MoMALogger::debug("") << "=====";
      g[i].print();
...
arma::vec FusedGroups::find_beta_at(double target_lam) {
  ...
  for (int i = 0; i != NO_NEXT;) {
    double betaj =
        line_value_at(g[i].lambda, g[i].beta, g[i].slope, target_lam);
...
double FusedGroups::lines_meet_at(double x1, double x2, double slope1,
                                  double slope2, double y1, double y2) {
...
void FusedGroups::merge() {
  ...
  if (!is_valid(dst) || src == NO_NEXT) {
    MoMALogger::error(
        "Only valid groups can be merged: merge point is not valid");
  }
  if (dst >= src) {
  ...
  g[dst].slope = -1 / double(this->group_size(dst) + this->group_size(src)) *
                 (sgn1 + sgn2);
  ...
  if (pre_group != NO_PRE) {
    double lambda_pre =
        lines_meet_at(g[pre_group].lambda, g[dst].lambda, g[pre_group].slope,
                      g[dst].slope, g[pre_group].beta, g[dst].beta);
    heap.change_lambda_by_id(g[pre_group].map_to_heap, lambda_pre, this);
  }
  if (next_group != NO_NEXT) {
    double lambda_next =
        lines_meet_at(g[next_group].lambda, g[dst].lambda, g[next_group].slope,
...
bool FusedGroups::all_merged() { return heap.is_empty(); };
