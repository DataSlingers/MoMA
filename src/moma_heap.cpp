...
bool gt(const HeapNode &left, const HeapNode &right) { return left > right; }
...
int Heap::min_child(int i) {
  ...
  if (child >= cur_size) {
    // no children
    return NO_CHILD;
  } else if (child + 1 >= cur_size ||
             !(heap_storage[child] > heap_storage[child + 1])) {
...
void Heap::swap(int i, int j, FusedGroups *fg) {
  // // DEBUG INFO
  MoMALogger::debug("Swapping ")
      << heap_storage[i].lambda << "and " << heap_storage[j].lambda;
...
void Heap::siftdown(int current_node, FusedGroups *fg) {
  ...
  while (child != NO_CHILD &&
         (heap_storage[current_node] > heap_storage[child])) {
...
int Heap::change_lambda_by_id(int i, double new_lambda, FusedGroups *fg) {
  if (i < 0 || i >= heap_storage.size()) {
    MoMALogger::error("Try to change lambda: no such id in current heap: ")
        << i;
  }
...
void Heap::remove(int i, FusedGroups *fg) {
  ...
  (*fg).g[heap_storage[heap_storage.size() - 1].id].map_to_heap =
      FusedGroups::NOT_IN_HEAP;
  ...
  while (2 * i + 1 < heap_storage.size()) {
    ...
    if (2 * i + 2 < heap_storage.size()) {
      if (heap_storage[i] > heap_storage[2 * i + 2]) {
        MoMALogger::warning("Not a min-heap") << heap_storage[i].lambda << "and"
                                              << heap_storage[2 * i + 2].lambda;
        return 0;
      }
    }
    i++;
...
bool Heap::is_empty() { return heap_storage.size() == 0; }
