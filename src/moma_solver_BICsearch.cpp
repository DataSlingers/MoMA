...
Rcpp::List BIC_searcher::search(
    ...
    const arma::vec &alpha_u, const arma::vec &lambda_u) {
  ...
  MoMALogger::message("BIC = ") << minbic_u << ", chosen (alpha,lambda) = ("
                                << opt_alpha_u << ", " << opt_lambda_u << ").";

  return Rcpp::List::create(Rcpp::Named("lambda") = opt_lambda_u,
