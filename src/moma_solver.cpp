...
arma::vec _PR_solver::normalize(const arma::vec &u) {
  arma::vec res = u;
  double mn =
      is_S_idmat ? arma::norm(u) : arma::as_scalar(arma::sqrt(u.t() * S * u));
  if (mn > 0) {
    res /= mn;
  } else {
    res.zeros();
...
_PR_solver::_PR_solver(double i_alpha, const arma::mat &i_Omega,
                       double i_lambda, Rcpp::List prox_arg_list, double i_EPS,
                       int i_MAX_ITER, int i_dim)
    : dim(i_dim),
      lambda(i_lambda),
      alpha(i_alpha),
...
arma::vec _PR_solver::g(const arma::vec &v, const arma::vec &y,
...
double _PR_solver::bic(arma::vec y, const arma::vec &est) {
  ...
  return std::log(res * res / dim) +
         std::log(dim) / dim * df;  // ignore some constants here
}
...
PR_solver::PR_solver(const std::string &algorithm_string, double i_alpha,
                     const arma::mat &i_Omega, double i_lambda,
                     Rcpp::List prox_arg_list, double i_EPS, int i_MAX_ITER,
                     int dim) {
  if (algorithm_string.compare("ISTA") == 0) {
    prs = new ISTA(i_alpha, i_Omega, i_lambda, prox_arg_list, i_EPS, i_MAX_ITER,
                   dim);
  } else if (algorithm_string.compare("FISTA") == 0) {
    prs = new FISTA(i_alpha, i_Omega, i_lambda, prox_arg_list, i_EPS,
                    i_MAX_ITER, dim);
  } else if (algorithm_string.compare("ONESTEPISTA") == 0) {
    prs = new OneStepISTA(i_alpha, i_Omega, i_lambda, prox_arg_list, i_EPS,
                          i_MAX_ITER, dim);
  } else {
    MoMALogger::error("Your choice of algorithm not provided: ")
        << algorithm_string;
  }
};
