...
Rcpp::List cpp_sfpca(
    ...
    double EPS_inner, long MAX_ITER_inner, std::string solver, int k = 1) {
  ...
  int n_more_than_one = int(n_lambda_v > 1) + int(n_lambda_u > 1) +
                        int(n_alpha_u > 1) + int(n_alpha_v > 1);
  if (n_more_than_one > 0) {
    MoMALogger::error(
        "We don't allow a range of parameters in finding a rank-k svd.");
  }
  // store results
  ...
  return Rcpp::List::create(
      Rcpp::Named("lambda_u") = lambda_u, Rcpp::Named("lambda_v") = lambda_v,
      Rcpp::Named("alpha_u") = alpha_u, Rcpp::Named("alpha_v") = alpha_v,
      Rcpp::Named("u") = U, Rcpp::Named("v") = V, Rcpp::Named("d") = d);
}
...
Rcpp::List cpp_sfpca_grid(
      ...
      for (int k = 0; k < n_alpha_u; k++) {
        for (int m = 0; m < n_alpha_v; m++) {
          MoMALogger::info("Setting up model:")
              << " lambda_u " << lambda_u(i) << " lambda_v " << lambda_v(j)
              << " alpha_u " << alpha_u(k) << " alpha_v " << alpha_v(m);
          ...
          d(problem_id) =
              arma::as_scalar(problem.u.t() * problem.X * problem.v);
  ...
  if (problem_id != n_total) {
    MoMALogger::error(
        "Internal error: solution not found for all grid points.");
  }
  return Rcpp::List::create(
      Rcpp::Named("lambda_u") = lambda_u, Rcpp::Named("lambda_v") = lambda_v,
      Rcpp::Named("alpha_u") = alpha_u, Rcpp::Named("alpha_v") = alpha_v,
