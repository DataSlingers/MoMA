...
MoMA::MoMA(const arma::mat &i_X,  // Pass X_ as a reference to avoid copy
            ...
            */
           double i_EPS, long i_MAX_ITER, double i_EPS_inner,
           long i_MAX_ITER_inner, std::string i_solver)
    : n(i_X.n_rows),
      p(i_X.n_cols),
      alpha_u(i_alpha_u),
      alpha_v(i_alpha_v),
      ...
      MAX_ITER(i_MAX_ITER),
      EPS(i_EPS),
      solver_u(i_solver, alpha_u, Omega_u, lambda_u, i_prox_arg_list_u,
               i_EPS_inner, i_MAX_ITER_inner, i_X.n_rows),
      solver_v(i_solver, alpha_v, Omega_v, lambda_v, i_prox_arg_list_v,
...
// const reference must be passed to initializer list
{
  ...
  MoMALogger::info("Initializing MoMA object:")
      << " lambda_u " << lambda_u << " lambda_v " << lambda_v << " alpha_u "
      << alpha_u << " alpha_v " << alpha_v << " P_u "
      << Rcpp::as<std::string>(i_prox_arg_list_u["P"]) << " P_v "
      << Rcpp::as<std::string>(i_prox_arg_list_v["P"]);
...
Rcpp::List MoMA::select_nestedBIC(
  ...
  while (tol > EPS && iter < MAX_ITER && iter < max_bic_iter) {
    ...
    for (int i = 0; i < alpha_u.n_elem; i++) {
      for (int j = 0; j < lambda_u.n_elem; j++) {
        ...
        MoMALogger::debug("(now,min,la,al) = (")
            << working_bic_u << "," << minbic_u << "," << lambda_u(j) << ","
            << alpha_u(i) << ")";
    ...
    MoMALogger::message("Search No.")
        << iter << ", BIC(u) = " << minbic_u << ", (al,lam) = (" << opt_alpha_u
        << ", " << opt_lambda_u << ").";
    ...
    for (int i = 0; i < alpha_v.n_elem; i++) {
      for (int j = 0; j < lambda_v.n_elem; j++) {
        ...
        MoMALogger::debug("(now,min) = (")
            << working_bic_v << "," << minbic_v << "," << lambda_v(j) << ","
            << alpha_v(i) << ")";
    ...
    MoMALogger::message("Search No.")
        << iter << ", BIC(v) = " << minbic_v << ", (al,lam) = (" << opt_alpha_v
        << ", " << opt_lambda_v << ").";
  ...
  solve();
  return Rcpp::List::create(Rcpp::Named("lambda_u") = opt_lambda_u,
...
int MoMA::check_cnvrg() {
  if (iter >= MAX_ITER) {
    MoMALogger::warning("No convergence in MoMA!")
        << " lambda_u " << lambda_u << " lambda_v " << lambda_v << " alpha_u "
        << alpha_u << " alpha_v " << alpha_v;
  }
  return 0;
}

int MoMA::reset(double newlambda_u, double newlambda_v, double newalpha_u,
                double newalpha_v) {
...
Rcpp::List MoMA::grid_BIC_mix(
    ...
    int selection_criterion_lambda_v, int max_bic_iter) {
  ...
  const arma::vec &grid_lu =
      set_greedy_grid(lambda_u, !selection_criterion_lambda_u);
  const arma::vec &grid_lv =
      set_greedy_grid(lambda_v, !selection_criterion_lambda_v);
  const arma::vec &grid_au =
      set_greedy_grid(alpha_u, !selection_criterion_alpha_u);
  const arma::vec &grid_av =
      set_greedy_grid(alpha_v, !selection_criterion_alpha_v);

  // Test that if a grid is set to be BIC-search grid, then
  // the above code should set grid_xx to the vector [-1]
  if ((selection_criterion_alpha_u == 1 &&
       (grid_au.n_elem != 1 || grid_au(0) != -1)) ||
      (selection_criterion_alpha_v == 1 &&
       (grid_av.n_elem != 1 || grid_av(0) != -1)) ||
      (selection_criterion_lambda_u == 1 &&
       (grid_lu.n_elem != 1 || grid_lu(0) != -1)) ||
      (selection_criterion_lambda_v == 1 &&
       (grid_lv.n_elem != 1 || grid_lv(0) != -1))) {
    MoMALogger::error("Wrong grid-search grid!")
        << "grid_lu.n_elem=" << grid_lu.n_elem
        << ", grid_av.n_elem=" << grid_av.n_elem << ", grid_lu.n_elem"
        << grid_lu.n_elem << ", grid_lv.n_elem" << grid_lv.n_elem;
  }
      ...
      for (int k = 0; k < n_alpha_v; k++) {
        for (int m = 0; m < n_lambda_v; m++) {
          arma::vec bic_au_grid =
              set_bic_grid(alpha_u, selection_criterion_alpha_u, i);
          arma::vec bic_lu_grid =
              set_bic_grid(lambda_u, selection_criterion_lambda_u, j);
          arma::vec bic_av_grid =
              set_bic_grid(alpha_v, selection_criterion_alpha_v, k);
          arma::vec bic_lv_grid =
              set_bic_grid(lambda_v, selection_criterion_lambda_v, m);
