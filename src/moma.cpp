...
MoMA::MoMA(const arma::mat &i_X, // Pass X_ as a reference to avoid copy
    ...
    */
           ...
           std::string i_solver) : n(i_X.n_rows),
                                   p(i_X.n_cols),
                                   alpha_u(i_alpha_u),
                                   alpha_v(i_alpha_v),
                                   lambda_u(i_lambda_u),
...
{ // suggested in the sfpca_nested_bic.m
    ...
    while (tol > EPS && iter < MAX_ITER && iter < max_bic_iter)
    {
        ...
        MoMALogger::message("Search No.") << iter << ", BIC(u) = " << minbic_u << ", (al,lam) = (" << opt_alpha_u << ", " << opt_lambda_u << ").";
        ...
        MoMALogger::message("Search No.") << iter << ", BIC(v) = " << minbic_v << ", (al,lam) = (" << opt_alpha_v << ", " << opt_lambda_v << ").";
                              ...
                              int max_bic_iter)
{
    ...
    if ((selection_criterion_alpha_u == 1 && (grid_au.n_elem != 1 || grid_au(0) != -1)) || (selection_criterion_alpha_v == 1 && (grid_av.n_elem != 1 || grid_av(0) != -1)) || (selection_criterion_lambda_u == 1 && (grid_lu.n_elem != 1 || grid_lu(0) != -1)) || (selection_criterion_lambda_v == 1 && (grid_lv.n_elem != 1 || grid_lv(0) != -1)))
    {
        ...
        for (int j = 0; j < n_lambda_u; j++)
        {
            for (int k = 0; k < n_alpha_v; k++)
            {
                for (int m = 0; m < n_lambda_v; m++)
                {
                    ...
                    if ((selection_criterion_alpha_u == 0 && bic_au_grid.n_elem != 1) || (selection_criterion_alpha_v == 0 && bic_av_grid.n_elem != 1) || (selection_criterion_lambda_u == 0 && bic_lu_grid.n_elem != 1) || (selection_criterion_lambda_v == 0 && bic_lv_grid.n_elem != 1))
                    {
