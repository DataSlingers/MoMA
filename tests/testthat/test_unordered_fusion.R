...
test_that("Find means of everything when lambda is large enough and the graph is fully connected",
          {
              ...
              for (p in c(5, 30, 40, 100)) {
                  ...
                  for (i in 1:rep) {
                      res.AMA = test_prox_fusion(y,
                                                 large.lambda,
                                                 w,
                                                 ADMM = FALSE,
                                                 acc = FALSE)
                      res.AMA.acc = test_prox_fusion(y,
                                                     large.lambda,
                                                     w,
                                                     ADMM = FALSE,
                                                     acc = TRUE)
                      res.ADMM = test_prox_fusion(y,
                                                  large.lambda,
                                                  w,
                                                  ADMM = TRUE,
                                                  acc = FALSE)
...
test_that("Find means of connected components when lambda is large enough", {
    ...
    for (p in c(9, 99)) {
        ...
        w <- matrix(rep(0, p * p), p, byrow = T)
        num.comp.nodes <-
            p / 3 # the number of nodes in each component
        for (i in seq(1, p, num.comp.nodes)) {
            w[i, (i + 1):(i + num.comp.nodes - 1)] <-
                1 + runif(num.comp.nodes - 1)
        }

        for (j in 1:rep) {
            y <- 10 * runif(p)
            res.AMA.unacc = test_prox_fusion(y,
                                             large.lambda,
                                             w,
                                             ADMM = FALSE,
                                             acc = TRUE)
...
test_that("Ordered fused lasso when w_ij = 1 all j = i+1", {
    ...
    for (p in c(3, 20, 100)) {
        ...
        for (i in 1:rep) {
            ...
            err.AMA.unacc <-
                norm(
                    test_prox_fusedlassodp(y, 1) - test_prox_fusion(y, 1, w, ADMM = FALSE, acc =
                                                                        TRUE)
                )
            err.AMA.acc <-
                norm(
            ...
            err.ADMM <-
                norm(
                    test_prox_fusedlassodp(y, 1) - test_prox_fusion(y, 1, w, ADMM = TRUE, acc =
                                                                        FALSE)
                )
