...
test_that("Error when accessing broundary", {
    ...
    expect_error(
        get_4Dlist_elem(result, n_alpha_u, n_lambda_u, n_alpha_v, 0),
        "Invalid index \\(7,3,5,0\\), dim = c\\(7, 3, 5, 2\\)"
    )

    expect_error(
        get_4Dlist_elem(result, n_alpha_u, n_lambda_u, 0, n_lambda_v),
        "Invalid index \\(7,3,0,2\\), dim = c\\(7, 3, 5, 2\\)"
    )

    expect_error(
        get_4Dlist_elem(result, n_alpha_u, 0, n_alpha_v, n_lambda_v),
        "Invalid index \\(7,0,5,2\\), dim = c\\(7, 3, 5, 2\\)"
    )

    expect_error(
        get_4Dlist_elem(result, 0, n_lambda_u, n_alpha_v, n_lambda_v),
        "Invalid index \\(0,3,5,2\\), dim = c\\(7, 3, 5, 2\\)"
    )


    expect_error(
        get_4Dlist_elem(result, n_alpha_u, n_lambda_u, n_alpha_v, n_lambda_v + 1),
        "Invalid index \\(7,3,5,3\\), dim = c\\(7, 3, 5, 2\\)"
    )

    expect_error(
        get_4Dlist_elem(result, n_alpha_u, n_lambda_u, n_alpha_v + 1, n_lambda_v),
        "Invalid index \\(7,3,6,2\\), dim = c\\(7, 3, 5, 2\\)"
    )

    expect_error(
        get_4Dlist_elem(result, n_alpha_u, n_lambda_u + 1, n_alpha_v, n_lambda_v),
        "Invalid index \\(7,4,5,2\\), dim = c\\(7, 3, 5, 2\\)"
    )

    expect_error(
        get_4Dlist_elem(result, n_alpha_u + 1, n_lambda_u, n_alpha_v, n_lambda_v),
