...
arg_list <- list(
    ...
    alpha_u = alpha_u,
    alpha_v = alpha_v,
    Omega_u = second_diff_mat(3),
    Omega_v = second_diff_mat(4),
    lambda_u = lambda_u,
    lambda_v = lambda_v,
    prox_arg_list_u = add_default_prox_args(lasso()),
    prox_arg_list_v = add_default_prox_args(empty()),
    EPS = 1e-6,
    MAX_ITER = 1e+4,
    EPS_inner = 1e-6,
    MAX_ITER_inner = 1e+4,
...
test_that("BIC search returns correct-sized grid: four grid requests", {
    ...
    lv = sapply(result, function(x)
        x$v$lambda)
    av = sapply(result, function(x)
        x$v$alpha)
    lu = sapply(result, function(x)
        x$u$lambda)
    au = sapply(result, function(x)
        x$u$alpha)

    expect_equal(dim(result),
                 c(n_alpha_u, n_lambda_u, n_alpha_v, n_lambda_v))
    ...
    expect_equal(lu, rep(lambda_u, n_alpha_u,              each = n_lambda_v *
                             n_alpha_v))
    expect_equal(au, rep(alpha_u,  1,                      each = n_lambda_v *
                             n_alpha_v * n_lambda_u))
})
...
test_that("BIC search returns correct-sized grid: three grid requests", {
    ...
    # Loop order in C++ is (outmost) au, lu, av, lv (innermost)
    av = sapply(result2, function(x)
        x$v$alpha)
    lu = sapply(result2, function(x)
        x$u$lambda)
    au = sapply(result2, function(x)
        x$u$alpha)
...
test_that("BIC search returns correct-sized grid: two grid requests on u", {
    ...
    # Loop order in C++ is (outmost) au, lu, av, lv (innermost)
    lv = sapply(result3, function(x)
        x$v$lambda)
    av = sapply(result3, function(x)
        x$v$alpha)
    lu = sapply(result3, function(x)
        x$u$lambda)
    au = sapply(result3, function(x)
        x$u$alpha)
...
test_that("BIC search returns correct-sized grid: two grid requests on different sides",
          {
              ...
              # Loop order in C++ is (outmost) au, lu, av, lv (innermost)
              lv = sapply(result4, function(x)
                  x$v$lambda)
              av = sapply(result4, function(x)
                  x$v$alpha)
              lu = sapply(result4, function(x)
                  x$u$lambda)
              au = sapply(result4, function(x)
                  x$u$alpha)
