...
test_that("Correct prox match", {
    ...
    expect_output(
        moma_svd(
            matrix(runif(12), 3, 4),
            u_sparsity = grplasso(factor(seq(3))),
            lambda_u = 3
        ),
    ...
    expect_output(
        moma_svd(
            matrix(runif(12), 3, 4),
            u_sparsity = grplasso(factor(seq(3)), non_negative =
                                      TRUE),
            lambda_u = 3
        ),
...
test_that("Correct algorithm match", {
    ...
    expect_output(moma_svd(
        matrix(runif(12), 3, 4),
        u_sparsity = cluster(diag(3), ADMM = TRUE),
        lambda_u = 3
    ),
    "Running ADMM")
