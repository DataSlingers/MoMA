...
test_that(
    "A numeric example: Ordered fused lasso should return correct values under different lambdas",
    {
        set.seed(34)
        x <- 10 * runif(10)
        ...
        for (l in 1:10) {
            expect_lte(norm(
                test_prox_fusedlassopath(x, lambdas[l]) - matrix(goal[, l], nrow = 10)
            ), 1e-5)
        }
    }
)
            ...
            for (i in 1:10) {
                ...
                for (lambda in seq(0, 5, 0.5)) {
                    expect_equal(
                        ...
                        t(
                            flsa::flsaGetSolution(flsa::flsa(x), lambda2 = lambda)
                        )
                    )
                }
            }
